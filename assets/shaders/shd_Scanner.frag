...
void main() {
  ...
  //  uv = rotate(uv, PI/2.);
   ...
   // uv.x *= 1.5;// * sin(PI);
    ...
    float nBlended = 100.;//mix(noise(polarUV + vec2(0.0, 2.0)), n, blendFactor);
    
    nBlended += disEdge+100.5-nBlended;
    nBlended = 5./mix(nBlended, fbm(uv/nBlended)*500., disEdge);
